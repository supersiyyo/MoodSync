...
- **Host (Without Spotify):** "As a host, I want to create a room authenticated through Apple Music so that I can host a session without needing a Spotify account."
...
- **Precondition:** User is authenticated with an Apple Music account.
- **Flow:** User decides to host -> Prompts for room name & settings -> Apple Music authentication verified -> Room created.
- **Postcondition:** Session ID generated and stored in Firebase.
...
- **Precondition:** User is either in a room or a solo listener with Apple Music configured.
